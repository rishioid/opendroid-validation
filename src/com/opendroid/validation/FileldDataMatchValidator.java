	...
	public boolean validate() {
		...
		for (View view : list) {
			if (view instanceof EditText) {
				...
				valueList.add(txt.getText().toString().trim());
				if (index > 0) {
					Log.d("Field match : ", txt.getText().toString().trim()+" = "+valueList.get(index-1));
					if (txt != null
							&& !txt.getText().toString().trim().equals(valueList.get(index-1))) {
						if (highlight) {
